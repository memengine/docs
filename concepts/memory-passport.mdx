...
<Note>
Memory Passport is an optional cross-application feature, not part of the normal tenant-memory quickstart. If your application stores and retrieves context only inside its own workspace, use one tenant API key and a stable `external_user_id`; your users do not need a MemoryOS account or consent redirect.
</Note>
