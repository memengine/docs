...
MemoryOS is a memory infrastructure layer for AI products. It lets your AI remember the right things about the right user without your team rebuilding extraction, retrieval, ranking, lifecycle cleanup, conflict handling, domain schemas, permissions, and dashboard controls from scratch.

Most developers should start by choosing one path.
...
| A customer support bot, support copilot, or agent-assist product | [Support agent cookbook](/cookbooks/support-agent) | Customer support memory for open issues, issue history, communication preference, support type, sentiment, and escalation risk |
| HR, healthcare, agriculture, or another domain that is not available yet | [General Engine](/concepts/general-engine) | Production-safe generic memory today; move to a domain schema when that domain becomes available |
