...
There is no separate SDK for domain schemas. If your tenant enables EdTech, Healthcare, or another future domain, `add()` and `get()` remain the main integration path. Optional domain helper methods expose structured profile data for dashboards.
