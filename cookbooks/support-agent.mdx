...
description: "Combine governed customer context from MemoryOS with authoritative data and actions from your live support tools."
---
...
```text
MemoryOS provides governed customer context.
Your tools provide current business records and actions.
...
```bash
curl -X PATCH "https://api.memoryo.dev/v1/tenant/domain-schema" \
...
```bash
curl -X PATCH "https://api.memoryo.dev/v1/tenant/support-type" \
  -H "Authorization: ApiKey mem_your_api_key" \
  -H "Content-Type: application/json" \
  -d '{
    "support_type_mode": "single",
...
- Treat live tool results as authoritative for current status and actions.
