...
---
This guide covers **Memory Passport**, the optional user-controlled cross-agent memory flow in MemoryOS.
...
1. Open **Dashboard -> Cross-app sharing**
2. Click **Create Passport agent**
3. Enter the agent name. This is the only profile field required to start.
4. Open advanced settings only if you want to add a website, logo, description, or default requested categories.
...
```bash
curl -X POST https://api.memoryo.dev/v1/agents/global \
...
```bash
curl https://api.memoryo.dev/v1/agents/global/2beab5d0-741d-4ff8-9475-c29d9ea5a57e
```

The path value must be the UUID returned when the agent was registered. A
placeholder or malformed value is rejected with `422` instead of being treated
as an agent lookup.

## Step 1: Explain shared memory in your app

<Tip>The consent page gives your users control over their information. They can review requested permissions, grant or revoke access, and choose how conflicting context should be handled across applications.</Tip>
...
const CONSENT_BASE_URL =
  process.env.NEXT_PUBLIC_MEMORYOS_CONSENT_BASE_URL ?? "https://consent.memoryo.dev";
...
| `MEMORYOS_SUPPORT_AGENT_ID`, `MEMORYOS_BILLING_AGENT_ID`, etc. | Dashboard -> Cross-app sharing -> create agent |
| `MEMORYOS_API_URL` | MemoryOS API base URL (https://api.memoryo.dev) |
| `MEMORYOS_CONSENT_BASE_URL` | MemoryOS consent app URL (https://consent.memoryo.dev) |
...
const MEMORYOS_API_URL = process.env.MEMORYOS_API_URL ?? "https://api.memoryo.dev";
const MEMORYOS_API_KEY = process.env.MEMORYOS_API_KEY;
const CONSENT_BASE_URL = process.env.MEMORYOS_CONSENT_BASE_URL ?? "https://consent.memoryo.dev";
...
MEMORYOS_API_URL = os.getenv("MEMORYOS_API_URL", "https://api.memoryo.dev")
MEMORYOS_API_KEY = os.environ["MEMORYOS_API_KEY"]  # mem_..., server-side only
CONSENT_BASE_URL = os.getenv("MEMORYOS_CONSENT_BASE_URL", "https://consent.memoryo.dev")
...
Tenant Dashboard -> Passport Studio includes a manual connector-link generator. Use it only to check one user flow during development. Production apps should generate the connector link server-side when the signed-in user clicks your button.
...
```bash
curl -X POST https://api.memoryo.dev/v1/tenant/directory/register \
  -H "Content-Type: application/json" \
  -H "Authorization: ApiKey mem_..." \
  -d '{
...
GET https://acme.com/oauth/authorize
  ...
  &redirect_uri=https://consent.memoryo.dev/oauth/callback
...
```http
302 https://consent.memoryo.dev/oauth/callback
