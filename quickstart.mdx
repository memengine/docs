...
If you are building a domain-specific product, the same SDK calls below still apply after you enable the tenant domain. Use:

- [EdTech tutor cookbook](/cookbooks/edtech-tutor) for tutoring, learning, and exam-prep products
- [Support agent cookbook](/cookbooks/support-agent) for customer support AI
- [Domain schemas](/concepts/domain-schemas) to choose between General, EdTech, Support, and future domains
...
- if a domain schema is enabled for your tenant, `system_prompt_addition` can include domain-aware context without changing SDK code
...
For domain-specific products, also see:
